...
public class NcCurveAnimation : NcEffectAniBehaviour
{
	...
	public class NcInfoCurve
	{
		...
		public		enum APPLY_TYPE						{ NONE, POSITION, ROTATION, SCALE, MATERIAL_COLOR, TEXTUREUV, MESH_COLOR, P_POSITION};//wo mj P_POSITION
		public		static string[]	m_TypeName			= { "None", "Position", "Rotation", "Scale", "MaterialColor", "TextureUV", "MeshColor", "Position_parent"};//wo mj Position_parent
...
// 			target.m_ToValue		= m_ToValue;
		...
		public int GetValueCount()
		{
			switch (m_ApplyType)
			{
				...
				case NcInfoCurve.APPLY_TYPE.P_POSITION:	//wo mj moon
		...
		public void SetDefaultValueScale()
		{
			switch (m_ApplyType)
			{
				...
				case NcInfoCurve.APPLY_TYPE.P_POSITION:     m_fValueScale   = 1;    break;//wo mj moon
...
// 				range.xMax = Mathf.Max(range.yMax, m_AniCurve[n].time);
    ...
    public     bool m_loop = false;//~.~ NC_ loop
	...
	//mj Begin
	public		bool			m_LoopedStartRandomPosition = false;
	public		bool			m_parentAxis				= false;
	public		Vector3			m_RandomPos 				= new Vector3 (0, 0, 0);
...
//		Debug.Log("NcCurveAnimation.Start " + transform.parent.name);
		...
		m_OrgLocalPosition = m_Transform.localPosition;
	}
	...
	void InitAnimation()
	{
		...
		foreach (NcInfoCurve curveInfo in m_CurveInfoList)
		{
			...
			switch (curveInfo.m_ApplyType)
			{
				...
				//wo mj begin
				case NcInfoCurve.APPLY_TYPE.P_POSITION:
					{
	...
	void UpdateAnimation(float fElapsedRate)
	{
		...
		foreach (NcInfoCurve curveInfo in m_CurveInfoList)
		{
			...
			switch (curveInfo.m_ApplyType)
			{
				...
				case NcInfoCurve.APPLY_TYPE.P_POSITION:
				{
					curveInfo.m_bApplyOption[0] = true;
					float newValue = GetNextValue(curveInfo, 0, fValue);
					if(m_Transform.parent != null)
					{
						Vector3 localPositionNormal = m_Transform.localPosition.normalized;
						localPositionNormal *= newValue;
						m_Transform.localPosition += localPositionNormal;
					}
					else
					{
						Debug.LogError("NoHave Parent");
					}
					break;
				}
				//wo mj end
		...
		if (0 != m_fDurationTime)
		{
			if (1 < m_fElapsedRate)
			{
                ...
                if (m_loop)
                {
					...
					if(m_LoopedStartRandomPosition == true)
					{ 
						InitPosition();
						if(m_Transform != null)
						{
							...
							if(m_parentAxis)
							{
								l_position.x = Random.Range( -m_RandomPos.x, m_RandomPos.x);
								l_position.y = Random.Range( -m_RandomPos.y, m_RandomPos.y);
								l_position.z = Random.Range( -m_RandomPos.z, m_RandomPos.z);

							}
							else
							{
								l_position.x = Random.Range( m_OrgLocalPosition.x -m_RandomPos.x, m_OrgLocalPosition.x + m_RandomPos.x);
								l_position.y = Random.Range( m_OrgLocalPosition.y -m_RandomPos.y, m_OrgLocalPosition.y + m_RandomPos.y);
								l_position.z = Random.Range( m_OrgLocalPosition.z -m_RandomPos.z, m_OrgLocalPosition.z + m_RandomPos.z);
							}


							m_Transform.localPosition = l_position;
						} 
					} 
                }
                else
