...
// 			FxmInfoIndexing.FindInstanceIndexing(m_Sel.transform, false).GetComponent<NcCurveAnimation>().ResumeAnimation();
		...
		EditorGUI.BeginChangeCheck();
		{
            ...
            m_Sel.m_loop = EditorGUILayout.Toggle(GetHelpContent("m_Loop"), m_Sel.m_loop);//~.~ NC loop
			...
			//mj start
			m_Sel.m_LoopedStartRandomPosition = EditorGUILayout.Toggle("LoopedStartRandomLocalPosition", m_Sel.m_LoopedStartRandomPosition);
			if(m_Sel.m_LoopedStartRandomPosition )
			{
				m_Sel.m_parentAxis = EditorGUILayout.Toggle("Form Parent Postion", m_Sel.m_parentAxis);

				m_Sel.m_RandomPos = EditorGUILayout.Vector3Field("LoopedRandomLocalPosition", m_Sel.m_RandomPos);
			} 
			//mj end
